...
<%
	...
	count = dbPro.getOrderCount();							// 전체 상품 수
	if (count > 0) { // 글이 하나라도 있으면
		orderList = dbPro.getOrder(startRow, endRow); // getProducts 메소드 리턴값 List, 반복할 때마다 dto 만든 걸 List로 반환해둔 것
	}
...
<%  } else {    %>
<table width="1250" border="1" cellpadding="0" cellspacing="0" align="center"> <!-- cellpadding: 글 사이에 약간의 여백이 있는데 그걸 없앤다는 뜻 (table border 있어서) -->
	<tr height="30" > 
	    ...
	    <td align="center"  width="250" >주문명</td>
...
<%	// 전체 게시글 수가 0보다 크면 // 페이지 번호 보이는 개수 10개로 지정한 것 페이지 번호 11 이상이면 [이전]으로 나오게
    if (count > 0) { // 직접 계산해보기 // 계산해서 페이지 번호 보여주는 것 // 10 넘어가면 [이전]으로 나오고 11부터 시작
        ...
        if (startPage > 10) {    %>
        <a href="orderList.jsp?pageNum=<%= startPage - 10 %>">[이전]</a>
<%      }
        for (int i = startPage ; i <= endPage ; i++) {  %> <!-- 페이지 번호 1씩 증가 -->
        	<a href="orderList.jsp?pageNum=<%= i %>">[<%= i %>]</a>
<%		}
        if (endPage < pageCount) {  %> 
        	<a href="orderList.jsp?pageNum=<%= startPage + 10 %>">[다음]</a>
<%		}
    }
%><br>
