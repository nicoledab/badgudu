...
<%
	...
	getSearchCount = dao.getProductCount(search);
	...
	//상품 썸네일 가져오기
	List thumbnailImgList = dao.getThumbnailImg(startRow, endRow, search);
	String thumbnailImg = "";
		
	//상품 이름, 가격 가져오기
	List thumbnailPdList = null;
	thumbnailPdList = dao.getThumbnailPd(startRow, endRow, search);
...
<%
	if(getSearchCount>0){
		int pageCount = getSearchCount / pageSize + (getSearchCount%pageSize == 0 ? 0:1);
		...
		if (startPage>10){
			%> <a href="searchList.jsp?pageNum=<%=startPage-10 %>&search=<%=search%>"> [이전]</a><%
		}
		for(int i = startPage; i <= endPage; i++){
			%> <a href="searchList.jsp?pageNum=<%=i %>&search=<%=search%>">[<%=i %>]</a><%
		}
		if (endPage<pageCount) {
			%> <a href="searchList.jsp?pageNum=<%=startPage+10 %>&search=<%=search%>">[다음]</a><%
		}
	}
