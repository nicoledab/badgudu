...
<%
    ...
    count = dbPro.usergetArticleCount(col,search);
	
    if (count > 0) {
    	articleList = dbPro.usergetArticles(col, search, startRow, endRow);
    }
...
%>
   <tr bgcolor="#f7f7f7"
          ...
          onmouseover="this.style.backgroundColor = '#eeeeef'"> 
            <!-- 마우스 올렸을때랑 기본 색 지정-->
            <td align="center"> <input type="checkbox" name="usercheck"></td>
            <td align="center" width="50"> <%= number-- %></td>
             ...
             <td align="center" width="200"><%= article.getPhoneNumber() %>
         </tr>
   <%
      }%>
</table>
...
<%
    if (count > 0) {
      ...
      int pageBlock=10;
        ...
        if (startPage > 10) {    %>
        <a href="crmSearchList.jsp?pageNum=<%= startPage - 10 %>&col=<%=col%>&search=<%=search%>">[이전]</a>
<%      }
        for (int i = startPage ; i <= endPage ; i++) {  %>
           <a href="crmSearchList.jsp?pageNum=<%= i %>&col=<%=col%>&search=<%=search%>">[<%= i %>]</a>
<%      }
        if (endPage < pageCount) {  %>
           <a href="crmSearchList.jsp?pageNum=<%= startPage + 10 %>&col=<%=col%>&search=<%=search%>">[다음]</a>
<%      }
    }
%>
